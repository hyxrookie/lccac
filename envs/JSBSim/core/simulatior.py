...
class MissileSimulator(BaseSimulator):
    ...
    def run(self):  # 定义了导弹在仿真环境中每经过一个时间步长 dt 时的运行逻辑
        ...
        if distance < self._Rc and self.target_aircraft.is_alive:  # 如果导弹与目标的距离小于爆炸半径 _Rc 且目标飞机仍然存活，则认为导弹击中了目标。
            ...
            self.target_aircraft.shotdown()
                ...
                or np.sum(
            self._distance_increment) >= self._distance_increment.maxlen or not self.target_aircraft.is_alive:
    ...
    def _guidance(self):
        ...
        Rxy = np.linalg.norm(
            [x_m - x_t, y_m - y_t])  # distance from missile to target project to X-Y plane 导弹到目标在水平面（X-Y平面）上的投影距离。
...
class MissileSimulator(BaseSimulator):
    ...
    @classmethod
    def create(cls, parent: AircraftSimulator, target: AircraftSimulator, uid: str, missile_model: str = "AIM-9L"):
        # 创建并发射 在前面新增env来进行发射时间统计。singlecombatenv
        # 注意，先执行create，再执行init，设值导弹status为INACTIVE，
        # 紧接着执行launch发射，设置导弹status为LAUNCHED
        # 注意，这个过程时在执行完run之后发生的，假设此刻是n时刻，那么在n时刻时，导弹的run是不执行的，因为还没有创建导弹，
        # 在n时刻执行完run循环后，self.task.step(self)进入导弹创建入口，并执行该create，产生导弹，此时导弹仅仅在n时刻产生，并为发射，之后在n+1时刻，导弹才会执行run，进行制导
        assert parent.dt == target.dt, "integration timestep must be same!"
        missile = MissileSimulator(uid, parent.color, missile_model, parent.dt)  # 执行__init__
        missile.launch(parent)  # 发射
        missile.target(target)  # 设置目标
        # missile.launch_time = env.current_step-1 #因为计算奖励是从第0步开始的
        missile.hitFlag = 0
        missile.missFlag = 0
        missile.aliveFlag = 1
        return missile
    ...
    def __init__(self,
                 ...
                 dt=1 / 12):
        super().__init__(uid, color, dt)
        self.__status = MissileSimulator.INACTIVE  # 状态为未激活
        self.model = model
        self.parent_aircraft = None  # type: AircraftSimulator
        self.target_aircraft = None  # type: AircraftSimulator
        self.render_explosion = False
        ...
        # missile parameters (for AIM-9L)
        self._g = 9.81  # gravitational acceleration 重力加速度，值为 9.81 m/s^2。
        self._t_max = 110  # time limitation of missile life 导弹的飞行时间限制，设为 60 秒
        self._t_thrust = 6  # time limitation of engine  导弹发动机的工作时间限制，设为 4秒。
        self._Isp = 140  # average specific impulse 比冲，即火箭发动机有效利用燃料的能力，这里设为 120 秒。这是导弹发动机性能的关键参数。
        self._Length = 2.87  # 导弹的长度和直径，分别为 2.87 米和 0.127 米。
        self._Diameter = 0.1
        self._cD = 0.4  # aerodynamic drag factor 导弹的气动阻力系数，设为 0.4。这个值影响导弹在空气中飞行时所受到的阻力。
        self._m0 = 84  # mass, unit: kg 导弹的初始质量，设为 84 千克
        self._dm = 6  # mass loss rate, unit: kg/s 导弹飞行过程中的质量损失率，设为每秒 6 千克。这通常是指导弹发动机燃烧燃料导致的质量减少。
        self._K = 5  # proportionality constant of proportional navigation 比例导航中的比例常数，设为 3。这个值决定了导弹制导系统的敏感度。
        self._nyz_max = 40  # max overload 导弹能够承受的最大过载，设为 30g。
        self._Rc = 450  # radius of explosion, unit: m 导弹爆炸的有效半径，设为 300 米。
        self._v_min = 150  # minimun velocity, unit: m/s  导弹的最小飞行速度，设为 150 米/秒。如果导弹速度低于此值，可能无法正常工作。
    ...
    # def modify_jsbsim_runner_buffer(self , ):
    ...
    def run(self):  # 定义了导弹在仿真环境中每经过一个时间步长 dt 时的运行逻辑
        ...
        if distance < self._Rc and self.target_aircraft.is_alive:  # 如果导弹与目标的距离小于爆炸半径 _Rc 且目标飞机仍然存活，则认为导弹击中了目标。
            ...
            # 或者距离增加的次数达到 _distance_increment 队列的最大长度，
            # 或者目标飞机已经被击落（不存活），则认为导弹未命中。
            # 注意，经过调试发现，在一个时间步内导弹的run会执行多步，当在某一步击中敌机，执行if块，设置self.__status = MissileSimulator.HIT，
            # 但是之后剩下的run会执行elif块，因为满足了not self.target_aircraft.is_alive为true。之后把self.__status 设置为 MissileSimulator.MISS
            # 这种情况下，并不是导弹未命中敌机！
            self.__status = MissileSimulator.MISS
            self.missFlag = 1
            self.aliveFlag = 0
